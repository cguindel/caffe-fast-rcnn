...
namespace caffe {
  ...
  void BoxAnnotatorOHEMLayer<Dtype>::Forward_gpu(
    ...
    const Dtype* bottom_viewp;
    if (bottom.size()>=5) {
      bottom_viewp = bottom[4]->cpu_data();
    }
    ...
    Dtype* top_viewp;
    if (top.size() >= 3){
      top_viewp = top[2]->mutable_cpu_data();
      caffe_set(top[2]->count(), Dtype(-10), top_viewp); //TODO
    }
    ...
    for (int i = 0; i < num_rois_; i++) {
      ...
      if (number_left[batch_ind] > 0) {
        ...
        for (int j = 0; j < bbox_channels_; j++) {
          ...
          if (top.size() >=3){
            top_viewp[index] = bottom_viewp[index];
          }
        }
      }
    }
